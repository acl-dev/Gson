#ifndef __GSON_HELPER__false_type
...
template<class T>
struct is_bool : _Is_bool<typename 
	std::remove_cv<typename std::remove_pointer<T>::type>::type>
{
};

template<bool T>
struct _Is_object : std::false_type
{
};
...
static inline add_item(acl::json &json, acl::json_node &node, bool value)
{
...
static inline gson(acl::json &json, const std::map<K, V> &objects)
{
			...
			itr != objects.end(); ++itr)
	{
		node.add_child(json.create_node()
			.add_number(get_value(itr->first),
...
template<class K, class V>
typename std::enable_if<std::is_floating_point<
	typename std::remove_pointer<V>::type>::value, acl::json_node &>::type
static inline gson(acl::json &json, const std::map<K, V> &objects)
{
	acl::json_node &node = json.create_array();
	for(std::map<K, V>::const_iterator itr = objects.begin();
	...
	{
		node.add_child(json.create_node()
			.add_double(get_value(itr->first),
				get_value(itr->second))
		);
	}
	return node;
...
expand(acl::json_node &node, std::map<std::string, T> *objs)
{
	...
	while(itr)
	{
		...
		result = gson(*itr, &obj);
		if(result.first)
			objs->insert(std::make_pair(
				std::string(itr->tag_name()), obj));
		else
			break;
		itr = node.next_child();
	}
