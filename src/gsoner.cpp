...
#define GSON_EXCEPTION(E)\
	...
	gsoner::field_t::field_t(type_t t,
	...
	std::string gsoner::get_unpack_code(const std::string &obj_name, const field_t &field)
	{
	...
	bool gsoner::check_struct_end()
	{
		if(status_ == e_struct_begin)
		{
			...
			if(codes_[pos_] == ';')
			{
				...
				{
					objs_.insert(
						std::make_pair(current_obj_.name_, current_obj_));
					current_obj_.reset();
				}
				status_ = e_uninit;
			}
