...
std::string gsoner::get_pack_bson_code(const field_t &field)
{
	std::string str;
	str += tab_;
	...
	}else if (field.type_ == field_t::e_int64)
	{
		...
		str += tab_;
		str += "return false;\n";
	}else if (field.type_ == field_t::e_string ||
...
bool gsoner::check_member()
{
	...
	else if(first == "int64_t" ||
			first == "uint64_t")
	{
		field_t f;
		f.type_ = field_t::e_int64;
		f.name_ = name;
		f.required_ = required_;
		current_obj_.fields_.push_back(f);
	}
...
void gsoner::gen_bson()
{
	const char *namespace_start = "namespace acl\n{";
	const char *namespace_end = "\n}///end of acl.\n";

	write_source("#include \"bson.h\"\n");
	write_source(get_include_files());
	write_source("#include \"" + gen_header_filename_ + "\"\n");
